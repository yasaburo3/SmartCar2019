 ...
 *
 * @file      adc.c
 * @brief     provide APIs for configuring FTM
 * @author    Freescale
 * @date      2018-04-06
 *
 ********************************************************************************/
 ...
 *            ClockSource ftm clock source
 *            ClockPrescale prescale factor
 * @return    none
 ************************************************************/
void FTM_ClockSet(FTM_Type* pFTM, uint8_t u8ClockSource,
  ...
  u8Temp = (pFTM->SC & 0xE0);
  u8Temp |=
      (FTM_SC_CLKS(u8ClockSource & 0x3) | FTM_SC_PS(u8ClockPrescale & 0x7));
  pFTM->SC = u8Temp;
}
...
void FTM_PWMInit(FTM_Type* pFTM, uint8_t u8PWMModeSelect,
  ...
  } else if (FTM_PWMMODE_COMBINE ==
             u8PWMModeSelect) /* enable combine pwm mode */
  {
...
/************************************************************
 * @brief     general configuration to FTM_No to input capture mode, enable
 *interrupt
 * @param     pFTM point to one of three FTM base register address
...
void FTM_InputCaptureInit(FTM_Type* pFTM, uint8_t u8FTM_Channel,
  ...
  if (FTM_INPUTCAPTURE_RISINGEDGE ==
      u8CaptureMode) /* enable interrupt, capture on rising edge */
  {
    pFTM->CONTROLS[u8FTM_Channel].CnSC =
        FTM_CnSC_CHIE_MASK | FTM_CnSC_ELSA_MASK;
  } else if (FTM_INPUTCAPTURE_FALLINGEDGE ==
             u8CaptureMode) /* capture on falling edge */
  {
    pFTM->CONTROLS[u8FTM_Channel].CnSC =
        FTM_CnSC_CHIE_MASK | FTM_CnSC_ELSB_MASK;
  } else if (FTM_INPUTCAPTURE_BOTHEDGE ==
             u8CaptureMode) /* capture on rising or falling edge */
  {
    pFTM->CONTROLS[u8FTM_Channel].CnSC =
...
/************************************************************
 * @brief     general configuration to FTM_No to Dual Edge Capture mode to
 *measure the width or the period  of a pulse
 * @param     pFTM point to one of three FTM base register address
 *            ChannelPair ChannelPair number to be configured: 0, 2, 4
...
void FTM_DualEdgeCaptureInit(FTM_Type* pFTM, uint8_t u8ChannelPair,
  ...
  pFTM->CONTROLS[u8ChannelPair].CnSC &=
      ~FTM_CnSC_CHF_MASK; /* CH(n)F and CH(n + 1)F bits must be cleared first */
  ...
  } else if (FTM_INPUTCAPTURE_DUALEDGE_CONTINUOUS ==
             u8CaptureMode) /* continuouse mode */
  {
    pFTM->CONTROLS[u8ChannelPair].CnSC |= FTM_CnSC_MSA_MASK;
    pFTM->CONTROLS[u8ChannelPair + 1].CnSC |= FTM_CnSC_MSA_MASK;
  }
  ...
  pFTM->CONTROLS[u8ChannelPair].CnSC |=
      (u8Channel_N_Edge << 2); /* select detec edge */
...
void FTM_OutputCompareInit(FTM_Type* pFTM, uint8_t u8FTM_Channel,
  ...
  pFTM->MOD = FTM_MOD_INIT;
  pFTM->CONTROLS[u8FTM_Channel].CnSC =
      (FTM_CnSC_MSA_MASK | (u8CompareMode << 2)); /* select detec edge */
 ...
 * @param     pFTM point to one of three FTM base register address
 *            u8TriggerN select the hardware trigger source
 * @return    none
 ************************************************************/
 ...
 *more then 1 trigger
 * @param     pFTM point to one of three FTM base register address
 *            u8TriggerMask select the hardware trigger source combine
 *TRIG0~TREG2 (x000xxxx~x111xxxx)
 * @return    none
 ************************************************************/
...
/************************************************************
 * @brief     generate FTM2 hardware trigger 2,Note: please call
 *FTM_HardwareSyncCombine first
...
void FTM_PWMDeadtimeSet(FTM_Type* pFTM, uint8_t u8PrescalerValue,
  ...
  if (!(pFTM->MODE & FTM_MODE_WPDIS_MASK)) /* if write protection is enabled */
  {
    ...
    pFTM->DEADTIME = (FTM_DEADTIME_DTVAL(u8DeadtimeValue & 0x3F) |
                      FTM_DEADTIME_DTPS(u8PrescalerValue & 0x3));
...
void FTM_OutputMaskSet(FTM_Type* pFTM, uint8_t u8FTM_Channel) {
  ...
  if (pFTM->SYNC & FTM_SYNC_SYNCHOM_MASK) /* if PWM sync is needed */
  {
    ...
    {
      pFTM->SYNC |= FTM_SYNC_SWSYNC_MASK;
    } else if (pFTM->SYNCONF &
               FTM_SYNCONF_HWOM_MASK) /* if hardware sync is needed */
    {
      pFTM->SYNC |= FTM_SYNC_TRIG2_MASK;
  ...
  if (pFTM->SYNCONF & FTM_SYNCONF_SWOC_MASK) /* if PWM sync is needed */
  {
    ...
    if (pFTM->SYNCONF & FTM_SYNCONF_SWSOC_MASK) /* if software sync is needed */
    {
      pFTM->SYNC |= FTM_SYNC_SWSYNC_MASK; /* software sync */
               ...
               FTM_SYNCONF_HWSOC_MASK) /* if hardware sync is needed */
    {
      pFTM->SYNC |= FTM_SYNC_TRIG2_MASK;
      SIM->SOPT0 |= SIM_SOPT0_FTMSYNC_MASK; /* hardware sync */
    }
 ...
 * @param     pFTM point to one of three FTM base register address
 *            u8DebugMode debug mode select from 00 to 11
 * @return    none
 ************************************************************/
 ...
 * @param     pFTM point to one of three FTM base register address
 *            u8TOFNUM TOF numbers before setting TOF bit, 0~31
 * @return    none
 ************************************************************/
...
void FTM_InvertChannel(FTM_Type* pFTM, uint8_t u8ChannelPair) {
  ...
  if (pFTM->SYNCONF & FTM_SYNCONF_INVC_MASK) /* if PWM sync is needed */
  {
    pFTM->SYNCONF |=
        FTM_SYNCONF_SYNCMODE_MASK; /* recommend enhanced sync mode */
    if (pFTM->SYNCONF &
        FTM_SYNCONF_SWINVC_MASK) /* if software sync is needed */
    {
      pFTM->SYNC |= FTM_SYNC_SWSYNC_MASK; /* software sync */
    } else if (pFTM->SYNCONF &
               FTM_SYNCONF_HWINVC_MASK) /* if hardware sync is needed */
    {
      pFTM->SYNC |= FTM_SYNC_TRIG2_MASK;
...
/************************************************************
 * @brief     configure the FTM as specified control parameters, CnSC and CnV
 *not included
 * @param     pFTM point to one of three FTM base register address
 *            pConfig point to FTM general parameters
 * @return    none
 ************************************************************/
void FTM_Init(FTM_Type* pFTM, FTM_ConfigType* pConfig) {
  ...
  pFTM->SC |= pConfig->clk_source << 3 | pConfig->prescaler << 0 |
              pConfig->cpwms << 5 | pConfig->toie << 6;
  if (pConfig->toie) {
    if (pFTM0 == pFTM) {
      NVIC_ClearPendingIRQ(FTM0_IRQn);
      NVIC_EnableIRQ(FTM0_IRQn);
...
void FTM_DeInit(FTM_Type* pFTM) {
  ...
  if (pFTM0 == pFTM) {
    SIM->SCGC &= ~SIM_SCGC_FTM0_MASK;
    NVIC_SetPendingIRQ(FTM0_IRQn);
    NVIC_DisableIRQ(FTM0_IRQn);
  } else if (pFTM1 == pFTM) {
    SIM->SCGC &= ~SIM_SCGC_FTM1_MASK;
    NVIC_SetPendingIRQ(FTM1_IRQn);
    NVIC_DisableIRQ(FTM1_IRQn);
  } else if (pFTM2 == pFTM) {
    SIM->SCGC &= ~SIM_SCGC_FTM2_MASK;
    NVIC_SetPendingIRQ(FTM2_IRQn);
 ...
 * @param     pFTM point to one of three FTM base register address
 *            FTM_Channel FTM channel number
 *            pTFTMCH_Params point to FTM channel general parameters
 * @return    none
 ************************************************************/
void FTM_ChannelInit(FTM_Type* pFTM, uint8_t u8FTM_Channel,
                     FTM_ChParamsType pTFTMCH_Params) {
  if (pFTM0 == pFTM) {
    SIM->SCGC |= SIM_SCGC_FTM0_MASK;
  }
  ...
  if (pTFTMCH_Params.ctrl.bits.bMode == FTM_INPUT_CAPTURE) {
    ...
    pFTM->CONTROLS[u8FTM_Channel].CnSC |= FTM_CnSC_MSA_MASK |
                                          FTM_CnSC_MSB_MASK |
                                          pTFTMCH_Params.ctrl.bits.bPWMPol << 2;

  } else if (pTFTMCH_Params.ctrl.bits.bMode == FTM_INPUTCAPTURE_DUALEDGE) {
    pFTM->CONTROLS[u8FTM_Channel].CnSC |= pTFTMCH_Params.ctrl.bits.bEdge << 2;
    if (pTFTMCH_Params.ctrl.bits.bDualCapMode ==
        FTM_INPUTCAPTURE_DUALEDGE_ONESHOT) {
      pFTM->CONTROLS[u8FTM_Channel].CnSC &= ~FTM_CnSC_MSA_MASK;

    } else if (pTFTMCH_Params.ctrl.bits.bDualCapMode ==
               FTM_INPUTCAPTURE_DUALEDGE_CONTINUOUS) {
      pFTM->CONTROLS[u8FTM_Channel].CnSC |= FTM_CnSC_MSA_MASK;
    }
  }
...
/************************************************************
 * @brief     configure the FTMx_SYNCONF register including SW and HW Sync
 *selection
 * @param     pFTM point to one of three FTM base register address
 *            u32ConfigValue FTMx_SYNCONF register config value
 * @return    none
 ************************************************************/
...
/************************************************************
 * @brief     quickly initialize FTM module
 * @param     pFTM point to one of three FTM base register address
 *            u8Channel FTM channel number
 *            FTMChMode FTM channels mode
 *            freq FTM frequency
 * @return    none
 ************************************************************/
void FTM_QuickInit(FTM_Type* pFTM, uint8_t u8Channel, uint8_t FTMChMode,
  ...
  FTM_Config.modulo = (freq>1000)?BUS_CLOCK * 2 /freq - 1:BUS_CLOCK * 2 /128/freq - 1;
  FTM_Config.clk_source = FTM_CLOCK_SYSTEMCLOCK;
  FTM_Config.prescaler = (freq>1000)?FTM_CLOCK_PS_DIV1:FTM_CLOCK_PS_DIV128;
  FTM_Config.mode = 1;
  FTM_Config.toie = 0;

  FTM_ChParamsType FTM_Ch_Config = {0};
  FTM_Ch_Config.u16CnV = 0;
  FTM_Ch_Config.ctrl.bits.bMode = FTMChMode;
  FTM_Ch_Config.ctrl.bits.bPWMPol = FTM_PWM_HIGHTRUEPULSE;
  FTM_Ch_Config.ctrl.bits.bCHIE = 0;

  FTM_ChannelInit(pFTM, u8Channel, FTM_Ch_Config);
...
//                   uint16_t freq) {
//  FTM_ConfigType FTM_Config = {0};
//  FTM_Config.modulo = BUS_CLOCK / freq - 1;
//  FTM_Config.clk_source = FTM_CLOCK_SYSTEMCLOCK;
